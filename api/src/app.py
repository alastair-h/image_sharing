...
@app.get("/get_posts/{email}", status_code=HTTPStatus.OK)
async def get_posts(email: str, db: AsyncSession = Depends(get_async_session)) -> List[ImagePost]:
    ...
    posts_dto = [ImagePost.from_db_model(post) for post in posts]
    return posts_dto
